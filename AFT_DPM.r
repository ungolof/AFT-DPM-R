...
# - Entropy computation
entropy <- rep(NA, n_iter)
entropy <- rowSums(ifelse(N_k_tb>0, N_k_tb * log(N_k_tb), 0))
plot(c(1:n_iter), entropy, type='l')
lines(c(1:n_iter), entropy, type='l',col='gray')
(range(entropy[c(30001:50000)])[2] - range(entropy[c(30001:50000)])[1]) / max(entropy[c(30001:50000)])



load("C:/Users/z3532149/Desktop/LFCR/AFT-DP/Centered parametrization paper/AFT_hp1_sv1_Pred.RData")
entropy <- rep(NA, n_iter)
entropy <- rowSums(ifelse(N_k_tb>0, N_k_tb * log(N_k_tb), 0))
plot(c(1:n_iter), entropy, type='l', xlab='Iteration', ylab='Entropy')

load("C:/Users/z3532149/Desktop/LFCR/AFT-DP/Centered parametrization paper/AFT_MCMC_Cent_sv2_hp1.RData")
entropy <- rowSums(ifelse(N_k_tb>0, N_k_tb * log(N_k_tb), 0))
lines(c(1:n_iter), entropy, type='l',col='gray')
(range(entropy[c(30001:50000)])[2] - range(entropy[c(30001:50000)])[1]) / max(entropy[c(30001:50000)])

plot(c(1:50000), N_c_tb, type='l')
...
WAIC_pieces <- function(unit){ # returns first log(mean(f)) and then mean(log(f))
  ...
  for(m in 1:M_theta){
    pf_vector_num <- log(pi_mw_thn[m,]) + d_ci[unit,1] * dnorm(log(t_i[unit]), mean = sum(beta1_thn[m,] * X1_cov[unit,]) + theta_star_thn[m,seq(1,M*K-2,M)], sd = sqrt(sigma2_c_thn[m,1]), log = TRUE) + (1 - d_ci[unit,1]) * pnorm(log(t_i[unit]), mean = sum(beta1_thn[m,] * X1_cov[unit,]) + theta_star_thn[m,seq(1,M*K-2,M)], sd = sqrt(sigma2_c_thn[m,1]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,2] * dnorm(log(t_i[unit]), mean = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sd = sqrt(sigma2_c_thn[m,2]), log = TRUE) + (1 - d_ci[unit,2]) * pnorm(log(t_i[unit]), mean = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sd = sqrt(sigma2_c_thn[m,2]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,3] * dnorm(log(t_i[unit]), mean = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sd = sqrt(sigma2_c_thn[m,3]), log = TRUE) + (1 - d_ci[unit,3]) * pnorm(log(t_i[unit]), mean = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sd = sqrt(sigma2_c_thn[m,3]), lower.tail = FALSE, log.p = TRUE)  
    
    max_pf_n <- max(pf_vector_num)
    
    pf_vector_den <- log(pi_mw_thn[m,]) + d_ci[unit,2] * dnorm(log(t_i[unit]), mean = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sd = sqrt(sigma2_c_thn[m,2]), log = TRUE) + (1 - d_ci[unit,2]) * pnorm(log(t_i[unit]), mean = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sd = sqrt(sigma2_c_thn[m,2]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,3] * dnorm(log(t_i[unit]), mean = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sd = sqrt(sigma2_c_thn[m,3]), log = TRUE) + (1 - d_ci[unit,3]) * pnorm(log(t_i[unit]), mean = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sd = sqrt(sigma2_c_thn[m,3]), lower.tail = FALSE, log.p = TRUE)  
...
WAIC_pieces <- function(unit){ # returns first log(mean(f)) and then mean(log(f))
  ...
  for(m in 1:M_theta){
    pf_vector_num <- log(pi_mw_thn[m,]) + d_ci[unit,1] * dlnorm(t_i[unit], meanlog = sum(beta1_thn[m,] * X1_cov[unit,]) + theta_star_thn[m,seq(1,M*K-2,M)], sdlog = sqrt(sigma2_c_thn[m,1]), log = TRUE) + (1 - d_ci[unit,1]) * plnorm(t_i[unit], meanlog = sum(beta1_thn[m,] * X1_cov[unit,]) + theta_star_thn[m,seq(1,M*K-2,M)], sdlog = sqrt(sigma2_c_thn[m,1]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,2] * dlnorm(t_i[unit], meanlog = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sdlog = sqrt(sigma2_c_thn[m,2]), log = TRUE) + (1 - d_ci[unit,2]) * plnorm(t_i[unit], meanlog = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sdlog = sqrt(sigma2_c_thn[m,2]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,3] * dlnorm(t_i[unit], meanlog = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sdlog = sqrt(sigma2_c_thn[m,3]), log = TRUE) + (1 - d_ci[unit,3]) * plnorm(t_i[unit], meanlog = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sdlog = sqrt(sigma2_c_thn[m,3]), lower.tail = FALSE, log.p = TRUE)  
    
    max_pf_n <- max(pf_vector_num)
    
    pf_vector_den <- log(pi_mw_thn[m,]) + d_ci[unit,2] * dlnorm(t_i[unit], meanlog = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sdlog = sqrt(sigma2_c_thn[m,2]), log = TRUE) + (1 - d_ci[unit,2]) * plnorm(t_i[unit], meanlog = sum(beta2_thn[m,] * X2_cov[unit,]) + theta_star_thn[m,seq(2,M*K-1,M)], sdlog = sqrt(sigma2_c_thn[m,2]), lower.tail = FALSE, log.p = TRUE) + 
                                          d_ci[unit,3] * dlnorm(t_i[unit], meanlog = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sdlog = sqrt(sigma2_c_thn[m,3]), log = TRUE) + (1 - d_ci[unit,3]) * plnorm(t_i[unit], meanlog = sum(beta3_thn[m,] * X3_cov[unit,]) + theta_star_thn[m,seq(3,M*K  ,M)], sdlog = sqrt(sigma2_c_thn[m,3]), lower.tail = FALSE, log.p = TRUE)  
